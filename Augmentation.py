...
def augment_image(image, augmentations):
    # Applique l'ensemble des transformations définies et retourne
    # un dictionnaire
...
def augment_image_file(input_file, output_directory=None):
    ...
    try:
        with Image.open(input_file) as img:
            ...
            for aug_name, func in augmentations:
                try:
                    ...
                    output_file = os.path.join(
                        output_directory, f"{base_name}_{aug_name}{ext}"
                    )
            ...
            print(
                f"Augmentation terminée. Total d'images générées : "
                f"{total_augmented}"
            )
    except Exception as e:
...
def augment_dataset(input_directory, output_directory):
    ...
    for category in os.listdir(input_directory):
        ...
        if os.path.isdir(category_input_path):
            ...
            images = [
                f
                for f in os.listdir(category_input_path)
            ...
            for img_name in images:
                ...
                try:
                    ...
                    print(
                        f"Erreur lors du traitement de l'image {img_path}: {e}"
                    )
    print(
        f"Augmentation terminée. Total d'images générées : {total_augmented}"
    )


def augment_dataset_balanced(input_directory, output_directory):
    # Traite l'ensemble du dataset par catégories et augmente les images
    # de chaque catégorie de sorte
    # que chaque catégorie atteigne un nombre d'images égal au maximum trouvé
    # parmi les catégories.
    # Les images sont générées en appliquant les augmentations de manière
    # cyclique.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Applique plusieurs transformations sur une image unique "
                    "ou sur l'ensemble d'un dataset."
    )
    parser.add_argument(
        "input_path",
        type=str,
        help="Chemin vers l'image à augmenter OU vers le répertoire contenant "
             "les catégories d'images si '--all' est spécifié.",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Si précisé, traite l'ensemble du dataset (input_path doit alors"
             " être un répertoire) et équilibre les catégories.",
    )
    ...
    if args.all:
        ...
        if not os.path.isdir(args.input_path):
            print(
                "Erreur : en mode '--all', le chemin d'entrée doit être un "
                "répertoire."
            )
        else:
            # Si aucun dossier de sortie n'est défini, on crée un dossier
            # 'augmented' à côté du répertoire source
            output_directory = args.out_dir or os.path.join(
                os.path.dirname(args.input_path), "augmented"
            )
        ...
        if os.path.isdir(args.input_path):
            print(
                "Erreur : pour traiter un seul fichier, veuillez spécifier "
                "directement le chemin de l'image ou utiliser '--all' "
                "pour un dataset."
            )
        else:
